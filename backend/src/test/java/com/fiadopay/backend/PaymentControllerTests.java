...
class PaymentControllerTests {
    ...
    void postPaymentDeclinesWhenHighAmount() throws Exception {
        ...
        String decline = null;
        for (int i = 0; i < 40; i++) {
            MvcResult r = mockMvc.perform(get("/payments/" + id)
                    .header("Authorization", "Bearer x"))
                    .andExpect(status().isOk())
                    .andReturn();
            Map<?, ?> resp = objectMapper.readValue(r.getResponse().getContentAsString(), Map.class);
            finalStatus = String.valueOf(resp.get("status"));
            decline = String.valueOf(resp.get("declineReason"));
            if ("DECLINED".equals(finalStatus)) {
                break;
            }
            Thread.sleep(50);
        }
        org.junit.jupiter.api.Assertions.assertEquals("DECLINED", finalStatus);
        org.junit.jupiter.api.Assertions.assertTrue(decline.contains("HighAmount"));
    }
}